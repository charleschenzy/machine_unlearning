...
def MIA_attack(target_model, shadow_model, shadow_client_loaders, shadow_test_loader, FL_params, client_loaders):
    ...
    Parameters
    ----------
    shadow_model : LogisticRegression model
        shadow model.
...
def attack(target_model, attack_model, client_loaders, test_loader, FL_params):
    ...
    # unlearn_y represents the all these forgotten client samples with a real label of 1
    ...
    with torch.no_grad ():
        for _, (data, target) in enumerate (test_loader):
            ...
            if (test_X.shape[0] > N_unlearn_sample):
                break
    # Used to test the membership inference attack model
    ...
    with torch.no_grad ():
        for ii in range (len (shadow_client_loaders)):
            ...
            for batch_idx,(data, target) in enumerate (data_loader):
                ...
                #print('pred_4_mem = {}'.format(pred_4_mem.shape))
                #out = out.view(-1,2)
